...
class PlaylistMaker:
    def __init__(self, clientid, clientsecret, user):
        ...
        self.token = util.prompt_for_user_token(self.username, self.scope, client_id=self.cid, client_secret=self.secret, redirect_uri='http://localhost:8888/callback')
        if self.token:
    ...
    def cleverSearch(self, track, artist, i):
        ...
        if i==0:
            ...
            while len(titlewords)>mintitlewords or len(artistwords)>minartistwords:
                query = ' '.join(titlewords[0:-1 if len(titlewords)>mintitlewords else len(titlewords)]) + ' ' + ' '.join(artistwords[0:-1 if len(artistwords)>minartistwords else len(artistwords)])
                r = self.pauseSearch(query)
                if len(r['tracks']['items'])>0:
                    break
                else:
                    titlewords = titlewords[0:-1 if len(titlewords)>mintitlewords else len(titlewords)]
                    artistwords = artistwords[0:-1 if len(artistwords)>minartistwords else len(artistwords)]
        else:
            print('XXXXX:', query)
            return None
        r = self.pauseSearch(query)
    ...
    def addManyTracksToPlaylist(self, playlisturi, largeurilist):
        ...
        while i<len(largeurilist):
            self.sp.user_playlist_add_tracks(self.username, playlisturi, largeurilist[i:j])
            i=i+100
            j=j+100
            time.sleep(0.1)
